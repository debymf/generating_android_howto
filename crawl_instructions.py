### CODE FROM: https://github.com/google-research/google-research/blob/master/seq2act/data_generation/crawl_instructions.py
...
flags.DEFINE_string(
    "input_warc_dir", None, "The directory of the downloaded WARC files."
)
flags.DEFINE_string(
    "output_instruction_json", None, "The path of the generated instruction json file."
)
...
def _is_valid(url, inst):
    ...
    return set(url_words + instruction_words).intersection(phone_set) and set(
        instruction_words
    ).intersection(click_set)
...
def _parse_one_page(lines, stats, domain_stats):
  ...
  """
    ...
    domain_stats["DOMAIN_" + domain][COUNT_INST] += len(instructions)
    return [(url, index, instruction) for index, instruction in enumerate(instructions)]
  ...
  """
    ...
    for line in file_handler:
        if line.strip().startswith("WARC/1.0"):
            stats["Total"] += 1
            urls_and_instructions = _parse_one_page(
                lines_of_one_page, stats, domain_stats
            )
    ...
    urls_and_instructions = _parse_one_page(lines_of_one_page, stats, domain_stats)
    ...
    for warc_file in glob.glob(os.path.join(FLAGS.input_warc_dir, "*.warc.gz")):
        with open(warc_file, "rb") as f1:
            with gzip.open(f1, mode="rt", encoding="latin1") as f2:
                for (url, index, instruction) in extract_instructions_from_warc_file(
                    warc_file, f2
                ):
                    output = {
                        "file_name": warc_file,

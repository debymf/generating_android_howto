...
flags.DEFINE_enum(
    "sharding",
    "700-golden-in-shard0",
    ["hash", "700-golden-in-shard0"],
    "The way to split data to sharding",
)
flags.DEFINE_string("input_csv_file", None, "Input CSV file of labeled data.")
flags.DEFINE_string(
    "input_instruction_json_file", None, "Input Json file of downloaded instructions."
)
flags.DEFINE_string(
    "output_dir", None, "Output directory for generated tf example proto data."
)
...
def _annotation_to_actions(annotation_str):
    ...
    for action_str in action_str_list:
        ...
        (action.verb_start_pos, action.verb_end_pos) = [
            int(x) for x in parts[1].split(":")
        ]
        (action.object_desc_start_pos, action.object_desc_end_pos) = [
            int(x) for x in parts[2].split(":")
        ]
        (action.input_content_start_pos, action.input_content_end_pos) = [
            int(x) for x in parts[3].split(":")
        ]
  ...
  """
    ...
    for action in task["actions"]:
        ...
        if do_stats and task["agreement-count"] >= 2:
            distributions["longest_verb_str"][
                verb_str_position_seq[-1] - verb_str_position_seq[-2]
            ] += 1
        ...
        if do_stats and task["agreement-count"] >= 2:
            distributions["longest_obj_desc"][
                obj_desc_position_seq[-1] - obj_desc_position_seq[-2]
            ] += 1

        if not (
            action.input_content_start_pos == 0 and action.input_content_end_pos == 0
        ):
            ...
            if do_stats and task["agreement-count"] >= 2:
                distributions["longest_input_str"][
                    input_str_position_seq[-1] - input_str_position_seq[-2]
                ] += 1
        else:
            input_str_position_seq.extend([config.LABEL_DEFAULT_VALUE_INT] * 2)

    features["verb_id_seq"] = np.array(verb_id_seq, dtype=np.int64)
    features["verb_str_position_seq"] = np.array(verb_str_position_seq, dtype=np.int64)
    features["obj_desc_position_seq"] = np.array(obj_desc_position_seq, dtype=np.int64)
    features["input_str_position_seq"] = np.array(
        input_str_position_seq, dtype=np.int64
    )

    features["agreement_count"] = np.array([task["agreement-count"]], dtype=np.int64)
    ...
    if do_stats:
        ...
        counters["total_input_refs"] += (
            ...
            - input_str_position_seq.count(config.LABEL_DEFAULT_VALUE_INT)
        ) / 2
...
def _write_tasks_to_tf_example(id_tasks_dict, output_dir, num_shards, sharding):
  ...
  """
    ...
    if sharding == "hash":
        ...
        for task_id, tasks in id_tasks_dict.items():
            if (
                testing_count < GOLD_NUM_IN_SHARD0
                and tasks[0]["agreement-count"] == len(tasks)
                and len(tasks) >= 3
            ):
                for task in tasks:
...
def _read_tasks(input_csv_file, input_instruction_json_file):
    ...
    with open(input_instruction_json_file, "r") as f:
        for line in f:
            if line.strip():
                ...
                id_instruction_dict[
                    get_task_id(json_dict["index"], json_dict["url"])
    ...
    if instruction_not_found == 0:
        print(
            "All %s instructions match with annotations successfully."
            % instruction_found
        )
    else:
        print(
            "%s instructions match with annotations successfully." % instruction_found
        )
        print(
            "Warning: can not find instructions for %s annotations, probably you "
...
def _generate_commoncrawl_dataset():
    ...
    id_tasks_dict = _read_tasks(FLAGS.input_csv_file, FLAGS.input_instruction_json_file)

    _write_tasks_to_tf_example(
        id_tasks_dict, FLAGS.output_dir, FLAGS.num_shards, FLAGS.sharding
    )
